...
\sol{
  ...
  \qproof{
    ...
    However, since both $n$ and $1/2$ are positive, it follows from Exercise~4.2 part~(h) that $n \cdot (1/2) = n/2$ is positive also so that $n/2 \in \pints$.
    ...
    Moreover, we have $n+1 > n > 0$ since $n \in \pints$ and again $1/2 > 0$ so that $(n+1) \cdot (1/2) = (n+1)/2$ is positive by Exercise~4.2 part~(h).
    Thus $(n+1)/2 \in \pints$.
  }
  ...
  \qproof{
    ...
    For any $\vx = \seqfin{x}{n} \in X$ we have that $x_i \in A_i$ for $1 \leq i \leq n$.
    So set $f(x) = (\seqfin{x}{n-1}, x_n)$, which is clearly an element of $Y$.

    To see that $f$ is injective consider $\vx = \seqfin{x}{n}$ and $\vy = \seqfin{y}{n}$ in $X$ where $\vx \neq \vy$.
    It then follows that there must be an $i \in \intsfin{n}$ where $x_i \neq y_i$.
    Let $\vx' = \seqfin{x}{n-1}$ and $\vy' = \seqfin{y}{n-1}$ so that clearly $f(\vx) = (\vx', x_n)$ and $f(\vy) = (\vy', y_n)$.
    Now, if $i = n$, then clearly $f(\vx) = (\vx', x_n) \neq (\vy', y_n) = f(\vy)$ since $x_n = x_i \neq y_i = y_n$.
    On the other hand, if $i \neq n$ then it has to be that $i < n$, and hence $i \leq n-1$.
    It then follows that $\vx' = \seqfin{x}{n-1} \neq \seqfin{y}{n-1} = \vy'$ so that then $f(\vx) = (\vx', x_n) \neq (\vy', y_n) = f(\vy)$ again.
    Since $\vx$ and $\vy$ were arbitrary, this shows that $f$ is indeed injective.

    Now consider any $\vy = (\seqfin{x}{n-1}, x_n) \in Y$ and let $\vx = \seqfin{x}{n}$.
  ...
  \qproof{
    ...
    So, for any $\va \in A$, we have that $\va = \seqinf{a}$, where $a_i \in A_i$ for any $i \in \pints$.
    Then, for any $i \in \pints$, define $b_i = (a_{2i-1}, a_{2i})$ so that clearly $b_i \in A_{2i-1} \times A_{2i} = B_i$.
    We then have that $\vb = \seqinf{b} \in B_1 \times B_2 \times \cdots = B$.
    So set $f(\va) = \vb$ so that $f$ is a function from $A$ to $B$.

    To show that $f$ is injective, consider $\va = \seqinf{a}$ and $\va' = \seqinf{a'}$ in $A$ where $\va \neq \va'$.
    For each $i \in \pints$, define $b_i = (a_{2i-1}, a_{2i})$ and $b_i' = (a_{2i-1}', a_{2i}')$ as above and set $\vb = \seqinf{b}$ and $\vb' = \seqinf{b'}$ so that clearly $f(\va) = \vb$ and $f(\va') = \vb'$.
    ...
    It then follows that $f(\va) = \vb = \seqinf{b} \neq \seqinf{b'} = \vb' = f(\va')$ so that $f$ is injective since $\va$ and $\va'$ were arbitrary.

    Lastly, to show that $f$ is surjective, consider any $\vb \in B$ so that $\vb = \seqinf{b}$ where $b_i \in B_i = A_{2i-1} \times A_{2i}$ for every $i \in \pints$.
    ...
    Hence $a_j \in A_j$ for all $j \in \pints$ so that $\va = \seqinf{a} \in A$.
    Then, for any $i \in \pints$, we have $b_i = (a_i', a_i'') = (a_{2i-1}, a_{2i}) \in A_{2i-1} \times A_{2i} = B_i$ so that by definition $f(\va) = \vb = \seqinf{b}$.
...
\sol{
  (a)
  \qproof{
    Suppose that $\vb \in B$ so that $\vb = \seqinf{b}$ where $b_i \in B_i$ for every $i \in \pints$.
    Consider any such $i \in \pints$ so that $b_i \in B_i$.
    Then also $b_i \in A_i$ since $B_i \ss A_i$.
    Since $i$ was arbitrary, $b_i \in A_i$ for every $i \in \pints$ so that $\vb = \seqinf{b} \in A_1 \times A_2 \times \cdots = A$.
  ...
  \qproof{
    Suppose that $B \ss A$.
    Since $B \neq \es$, there is a $\vb' \in B$ so that $\vb' = \seqinf{b'}$ where $b_i' \in B_i$ for every $i \in \pints$.
    ...
    for any $j \in \pints$.
    Clearly we have that $b_j \in B_j$ for any $j \in \pints$ so that $\vb = \seqinf{b} \in B_1 \times B_2 \times \cdots = B$.
  ...
  \qproof{
    ...
    Then, since $A = A_1 \times A_2 \times \cdots$, it follows that $\va = \seqinf{a}$ where $a_i \in A_i$ for every $i \in \pints$.
  ...
  Then there is an $a_i \in A_i$ for every $i \in \pints$ so that $\va = \seqinf{a} \in A_1 \times A_2 \times \cdots = A$ so that then $A \neq \es$.
  ...
  \qproof{
    ...
    If $\vx \in A$ then it has to be that $\vx = \seqinf{x}$ where $x_i \in A_i$ for every $i \in \pints$.
  ...
  \qproof{
    ...
    It then follows that $\vx = \seqinf{x}$ where $x_i \in A_i$ for every $i \in \pints$ and $x_i \in B_i$ for every $i \in \pints$.
    ...
    Now consider any $\vx \in C$ so that $\vx = \seqinf{x}$ where $x_i \in C_i$ for any $i \in \pints$.
...
\sol{
  ...
  \qproof{
    ...
    Now, for any $\vx \in X^m$ we have that $\vx = \seqfin{x}{m}$ where each $x_i \in X$.
    Then define
    \gath{
      y_i = \begin{cases}
        x_i & 1 \leq i \leq m \\
    ...
    for $i \in \intsfin{n}$.
    Clearly $y_i \in X$ for every $i \in \intsfin{n}$ so that $\vy = \seqfin{y}{n} \in X^n$.
    Then set $f(\vx) = \vy$ so that $f : X^m \to X^n$.

    To show that $f$ is injective consider $\vx$ and $\vx'$ in $X^m$ so that $\vx = \seqfin{x}{m}$ and $\vx' = \seqfin{x'}{m}$ where both $x_i$ and $x_i'$ are of course in $X$ for any $i \in \intsfin{m}$.
    Also suppose that $\vx \neq \vx'$ so that it follows that there is an $i \in \intsfin{m}$ where $x_i \neq x_i'$.
    Let $\vy = \seqfin{y}{n} = f(\vx)$ and $\vy' = \seqfin{y'}{n} = f(\vx')$.
  ...
  \qproof{
    ...
    Then we have that $\va = \seqfin{a}{m}$ and $\vb = \seqfin{b}{n}$ where $a_i,b_j \in X$ for every $i \in \intsfin{m}$ and $j \in \intsfin{n}$.
    Then define
    \gath{
      y_k = \begin{cases}
        a_k & 1 \leq k \leq m \\
    ...
    Now set $g(\vx) = \vy = \seqfin{y}{m+n}$ so that clearly $g(\vx) \in X^{m+n}$ since each $y_k \in X$.
  ...
  \qproof{
    ...
    So, for any $\vx = \seqfin{x}{n} \in X^n$, define
    \gath{
      y_i = \begin{cases}
        x_i & 1 \leq i \leq n \\
        x_0 & n < i
    ...
    for any $i \in \pints$.
    Then set $h(\vx) = \vy = \seqinf{y}$ so that clearly $h(\vx) \in X^\w$.
    Thus $h$ is a function that maps $X^n$ into $X^\w$.

    To show that $h$ is injective, consider $\vx$ and $\vx'$ in $X^n$ where $\vx \neq \vx'$.
    Clearly we have that $\vx = \seqfin{x}{n}$ and $\vx' = \seqfin{x'}{n}$, and let $\vy = \seqinf{y} = h(\vx)$ and $\vy' = \seqinf{y'} = h(\vx')$.
    ...
    It then follows that $h(\vx) = \vy = \seqinf{y} \neq \seqinf{y'} = \vy' = h(\vx')$, which shows that $h$ is injective since $\vx$ and $\vx'$ were arbitrary.
  }

  (d) \partd
  \qproof{
    Consider any $\vx = (\va,\vb) \in X^n \times X^\w$ so that clearly $\va = \seqfin{a}{n} \in X^n$ and $\vb = \seqinf{b} \in X^\w$.
    ...
    We then of course set $k(\vx) = \vy = \seqinf{y}$ so that clearly $k(\vx) \in X^\w$.
    ...
    It then follows that $\va = \seqfin{a}{n}$, $\va' = \seqfin{a'}{n}$, $\vb = \seqinf{b}$, and $\vb' = \seqinf{b'}$, where every $a_i$, $a_i'$, $b_j$, and $b_j'$ are in $X$ (for $i \in \intsfin{n}$ and $j \in \pints$).
    Also, let $\vy = \seqinf{y} = k(\vx)$ and $\vy' = \seqinf{y'} = k(\vx')$.
    ...
    Hence in either case there is an $i \in \pints$ such that $y_i \neq y_i'$ so that $k(\vx) = \vy = \seqinf{y} \neq \seqinf{y'} = \vy' = k(\vx')$.
    This shows that $k$ is injective since $\vx$ and $\vx'$ were arbitrary.

    Now consider any $\vy = \seqinf{y} \in X^\w$ and set $a_i = y_i$ for any $i \in \intsfin{n}$ so that clearly $\va = \seqfin{a}{n} \in X^n$.
    Also, for any $j \in \pints$, let $b_j = y_{n+j}$ so that clearly $\vb = \seqinf{b} \in X^\w$.
    Let $\vx = (\va,\vb)$ so that clearly $x \in X^n \times X^\w$.
    Now set $\vy' = \seqinf{y'} = k(\vx)$ as defined above.
    Consider any $i \in \pints$.
    If $1 \leq i \leq n$ then $y_i' = a_i = y_i$ by the definition of $k$.
    If $n < i$ then $y_i' = b_{i-n} = y_{n+(i-n)} = y_i$.
    Hence $y_i' = y_i$ for every $i \in \pints$ so that $k(\vx) = \vy' = \seqinf{y'} = \seqinf{y} = \vy$, which shows that $k$ is surjective since $\vy$ was arbitrary.
  ...
  (e) \parte
  \qproof{
    Consider any $\vx = (\va,\vb) \in X^\w \times X^\w$ so that clearly $\va = \seqinf{a}$ and $\vb = \seqinf{b}$.
    Now define
    \gath{
      y_i = \begin{cases}
    ...
    Clearly we have that $y_i \in X$ for any $i \in \pints$ so that $\vy = \seqinf{y} \in X^\w$.
    ...
    Also set $\vy = \seqinf{y} = l(\vx)$ and $\vy' = \seqinf{y'} = l(\vx')$.
    Since $\vx \neq \vx'$, we have that either $\va \neq \va'$ or $\vb \neq \vb'$.
    ...
    Hence in either case we have $y_i' = y_i$ so that $l(\vx) = \vy' = \seqinf{y'} = \seqinf{y} = \vy$ since $i$ was arbitrary.
  ...
  (f) \partf
  \qproof{
    Consider any $\vx \in \parens{A^\w}^n$ so that $\vx = \seqfin{\vx}{n}$ where $\vx_i \in A^\w$ for any $i \in \intsfin{n}$.
    ...
    It then follows that $\vy = \seqinf{y} \in B^\w$.
    ...
    Then $\vx = \seqfin{\vx}{n}$ and $\vx' = \seqfin{\vx'}{n}$ where each $\vx_i$ and $\vx_i'$ are in $A^\w$ for $i \in \intsfin{n}$.
    ...
    Hence $y_k = x_{ij} \neq x_{ij}' = y_k'$ so that clearly $m(\vx) = \vy = \seqinf{y} \neq \seqinf{y'} = \vy' = m(\vx')$.
...
\sol{
  ...
  \qproof{
    ...
    Let $(a, a, \ldots)$ denote the sequence $\seqinf{x}$ where $x_i = a$ for all $i \in \pints$.
