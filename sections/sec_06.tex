...
\sol{
  ...
  \qproof{
    ...
    for $i \in \pints$, noting that when $i \neq 1$ we have $i > 1$ so that $i-1 \geq 1$, and thus $y_i = x_{i-1}$ is defined.
...
\sol{
  ...
  \qproof{
    ...
    for any $x \in A$.
    It is clear that $f$ is a function from $A$ to $\intsfin{n+1}$ since obviously $n+1 \in \intsfin{n+1}$ and the image of $f'$ is $\intsfin{n} \ss \intsfin{n+1}$.
    ...
    If $x' = a$ then $f(x') = n+1$ so that $f(x) = f'(x) \leq n < n+1 = f(x')$ since the image of $f'$ is only $\intsfin{n}$.
    ...
    Then $k \in \intsfin{n}$, which is the image of $f'$ so that there is an $x \in A'$ where $f'(x) = k$ since $f'$ is bijective (and therefore surjective).
