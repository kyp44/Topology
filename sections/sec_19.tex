...
\sol{
  ...
  (b)
  \qproof{
    Let $\cT'$ be the topology defined by the subbasis $\cS$ and let $\cT$ be the coarsest topology defined in part (a).
    We show that $\cT' = \cT$.

    $(\ss)$ Suppose that $U \in \cT'$ and consider any $x \in U$.
    Since $U$ is open set of $\cT'$ there is a basis element $B$ containing $x$ where $B \ss U$.
    We then have that $B$ is the finite intersection of subbasis elements, i.e. $B = \bigcap_{n \in I} S_n$, where each each $S_n$ is a subbasis element and $I$ is finite.
    Then each $S_n$ is the inverse image of an open set of $X_\b$ under one of the functions $f_\b$.
    It then follows based on the definition of the collection $\cC$ from part (a) that each $S_n$ is in every topology in $\cC$ since $f_\b$ is continuous relative to those topologies.
    Hence each $S_n \in \bigcap \cC = \cT$.
    Since $\cT$ is a topology, we then have that the finite intersection $B = \bigcap_{n \in I} S_n$ is in $\cT$ as well.
    Therefore $x \in B$, $B$ is open in the topology $\cT$, and $B \ss U$.
    Since $x$ was arbitrary, this shows that $U$ is open relative to $\cT$ by Exercise~13.1.
    Thus $U \in \cT$ so that $\cT' \ss \cT$ since $U$ was arbitrary.

    $(\sps)$ Now consider any $U \in \cT$ so that
  }
}
