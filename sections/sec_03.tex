...
\exercise{5}{
  ...
  \ali{
    S &= \braces{\pptp{} \where y = x+1 \text{ and } 0 < x < 2} \,, \\
    S' &= \braces{\pptp{} \where y - x \text{ is an integer}} \,.
  }
  \eparts{
...
\sol{
  ...
  \qproof{
    ...
    Consider any $\pptp{} \in S$ so that $0 < x < 2$ and $y = x + 1$.
    Then $y - x = (x + 1) - x = 1$, which is of course an integer.
    Hence $\pptp{} \in S'$, and thus $S' \sps S$ since $\pptp{}$ was arbitrary.
  }
  ...
  \qproof{
    ...
    First, suppose that any $\pptp{} \in C$ and consider any $D \in A^*$ so that $\pptp{} \in D$.
    Then also $\pptp{} \in A \times A$ since $D$ is a relation on $A$ so that $D \ss A \times A$.
    This shows that $C \ss A \times A$ since $\pptp{}$ was arbitrary, and so $C$ is a relation on $A$.
    ...
    (Symmetric) Suppose that $\pptp{} \in C$ and consider any $D \in A^*$ so that also $\pptp{} \in D$.
    ...
    (Transitivity) Suppose that $\pptp{} \in C$ and $(y,z) \in C$.
    For any $D \in A^*$ we then have that both $\pptp{} \in D$ and $(y,z) \in D$.
  ...
  \ali{
    S_1 &= \braces{\pptp{} \where y = x} &
    S_4 &= \braces{\pptp{} \where y = x + 2 \text{ and } 0 < x < 1} \\
    S_2 &= S = \braces{\pptp{} \where y = x + 1 \text{ and } 0 < x < 2} &
    S_5 &= \braces{\pptp{} \where y = x - 2 \text{ and } 2 < x < 3} \,. \\
    S_3 &= \braces{\pptp{} \where y = x - 1 \text{ and } 1 < x < 3}
  }
  ...
  \qproof{
    ...
    $(\ss)$ Consider any $\pptp{} \in T$ and any $R \in S^*$ so that $R$ is an equivalence relation on $\reals$ that contains $S$.
    We then have the following cases:

    Case: $\pptp{} \in S_1$.
    Then of course $y = x$ so that $\pptp{} = (x,x) \in R$ since it is an equivalence relation and thus reflexive.

    Case: $\pptp{} \in S_2 = S$.
    Then of course $\pptp{} \in R$ since $R$ contains $S$.
    ...
    We then have that $\pptp{} \in R$ as well since $R$ is an equivalence relation and therefore symmetric.
    ...
    Since $R$ is an equivalence relation and therefore transitive, we have that $\pptp{} \in R$ as well.

    Case: $\pptp{} \in S_5$.
    Then we have $y = x-2$ and $2 < x < 3$.
    It then follows that $x = y+2$ and $0 < y = x-2 < 1$ so that $(y,x) \in S_4$.
    Then of course $(y,x) \in R$ by the previous case so that also $\pptp{} \in R$ since $R$ is an equivalence relation and therefore symmetric.

    Thus in every case $\pptp{} \in R$ so that also $\pptp{} \in \bigcap_{R \in S^*} R$ since $R$ was arbitrary.
    It then follows that $T \ss \bigcap_{R \in S^*} R$ since $\pptp{}$ was arbitrary.
...
\sol{
  ...
  \qproof{
    ...
    (Comparability) Consider distinct $\pptp{0}$ and $\pptp{1}$ in the plane so that either $x_0 \neq x_1$ or $y_0 \neq y_1$.
    ...
    (Nonreflexivity) Consider $\pptp{}$ in the plane so that obviously $\peq{} = \peq{}$.
    As we also have that $x = x$, it is not the case that $x < x$ so that it is not true that $\pptp{} < \pptp{}$.

    (Transitivity) Suppose that $\pptp{0} < \pptp{1}$ and $\pptp{1} < \pptp{2}$.
...
\sol{
  ...
  \gath{
    A = \braces{\pptp{} \where y = 1} \,.
  }
  We also claim that $(1,1)$ is the smallest element in $\pintss$ with this order.
  \qproof{
    First consider any point $\pptp{1} \in \pintss$ where $\pptp{1} \notin A$ so that $y_0 \neq 1$.
    It then follows that $y_1 > 1$ since $1$ is the smallest positive integer.
    Then set $x_0 = x_1$ and $y_0 = y_1 - 1$ so that clearly $\pptp{0} \in \pintss$ since $y_0 > 0$ because $y_1 > 1$.
    Clearly also $x_0 = x_1$ and $y_0 < y_1$ so that $\pptp{0} < \pptp{1}$ in the dictionary order.
    We claim that $\pptp{0}$ is the immediate predecessor of $\pptp{1}$.
    So suppose to the contrary that there is an $\pptp{2} \in \pintss$ such that $\pptp{0} < \pptp{2} < \pptp{1}$.
    It cannot be that $x_0 < x_2$ since then we would have $x_1 = x_0 < x_2$ so that $\pptp{1} < \pptp{2}$, which by Lemma~\ref{lem:relations:lngeq} contradicts the fact that $\pptp{2} < \pptp{1}$.
    So it has to be that $x_0 = x_2$ and $y_0 < y_2$.
    Since then $x_2 = x_0 = x_1$, it must also be that $y_2 < y_1$ since $\pptp{2} < \pptp{1}$.
    ...
    So it must be that no $\pptp{2}$ exists so that $\pptp{0}$ is the immediate predecessor of $\pptp{1}$.
    Thus every element not in $A$ has an immediate predecessor.

    Now consider any $\pptp{1} \in A$ so that $y_1 = 1$, and consider any point $\pptp{0} < \pptp{1}$.
    ...
    Then let $x_2 = x_0$ and $y_2 = y_0+1$ so that clearly $\pptp{2} \in \pintss$ since $y+1$ is always still a positive integer if $y$ is.
    Then we have that $x_0 = x_2$ and $y_0 < y_0 + 1 = y_2$, and hence $\pptp{0} < \pptp{2}$.
    We also have $x_2 = x_0 < x_1$ so that $\pptp{2} < \pptp{1}$.
    Therefore $\pptp{0} < \pptp{2} < \pptp{1}$, which shows that $\pptp{0}$ is not the immediate predecessor of $\pptp{1}$.
    This shows that $\pptp{1}$ has no immediate predecessor since $\pptp{0} < \pptp{1}$ was arbitrary.
    Since $\pptp{1} \in A$ was arbitrary, this completes the proof that every element in $\pintss$ has an immediate predecessor except those in $A$.

    It is easy to prove that $(1,1)$ is the smallest element in the dictionary order.
    Consider any $\pptp{0} \in \pintss$ and suppose that $\pptp{0} < (1,1)$.
    ...
    Thus it cannot be that $\pptp{0} < (1,1)$, so it must be that $(1,1) \leq \pptp{0}$ by Lemma~\ref{lem:relations:lngeq}.
    This shows that $(1,1)$ is the smallest element since $\pptp{0}$ was arbitrary.
  }
  ...
  \gath{
    A = \braces{\pptp{} \where x=1 \text{ or } y=1} \,.
  }
  We also claim that the set $\pintss$ has no smallest element in this order.
  \qproof{
    First consider any $\pptp{1} \notin A$ so that $x_1 \neq 1$ and $y_1 \neq 1$.
    Then it has to be that $x_1,y_1 > 1$.
    So set $\pptp{0} = (x_1-1,y_1-1)$ so that clearly still $\pptp{0} \in \pintss$.
    Then $x_0-y_0 = (x_1-1)-(y_1-1) = x_1-1-y_1+1 = x_1-y_1$.
    We also have $y_0 = y_1-1 < y_1$ so that $\pptp{0} < \pptp{1}$.
    Now suppose that there is an $\pptp{2} \in \pintss$ where $\pptp{0} < \pptp{2} < \pptp{1}$.
    It cannot be that $x_0-y_0 < x_2-y_2$ since then we would have $x_1-y_1 = x_0-y_0 < x_2-y_2$ so that $\pptp{1} < \pptp{2}$, which we know cannot be the case since $\pptp{2} < \pptp{1}$.
    So it must be that $x_2-y_2 = x_0-y_0 = x_1-y_1$ and $y_0 < y_2$, but then we must have $y_0 < y_2 < y_1 = y_0+1$, noting that $y_2 < y_1$ because $x_2-y_2 = x_1-y_1$ and $\pptp{2} < \pptp{1}$.
    However, this is not possible since of course $y_0+1$ is the immediate successor of $y_0$ in $\pints$.
    So then it has to be that no such $\pptp{2}$ exists so that $\pptp{0}$ is the immediate predecessor of $\pptp{1}$.
    This shows that every point not in $A$ has an immediate predecessor since $\pptp{1}$ was arbitrary.

    Now suppose that $\pptp{1} \in A$ so that $x_1 = 1$ or $y_1 = 1$, and consider any $\pptp{0} < \pptp{1}$.
  ...
  \qproof{
    First we show that $\pptp{0} = (1,1)$ is the smallest element, from which it follows that it cannot have an immediate predecessor since it has no predecessors at all.
    Consider any $\pptp{1}$ in $\pintss$.
    If $\pptp{1} = (1,1)$ then of course $\pptp{0} = (1,1) \leq \pptp{1}$ is true, so assume that $\pptp{1} \neq (1,1)$ so that either $x_1 \neq 1$ or $y_1 \neq 1$.
    If $x_1 \neq 1$ then it has to be that $x_1 > 1$ so that $x_0 + y_0 = 1 + 1 \leq 1 + y_1 < x_1 + y_1$, and hence $\pptp{0} < \pptp{1}$.
    If $y_1 \neq 1$ then $y_1 > 1$ so that again $x_0 + y_0 = 1 + 1 \leq x_1 + 1 < x_1 + y_1$, and hence $\pptp{0} < \pptp{1}$.
    Thus in every case $\pptp{0} \leq \pptp{1}$, which shows that $\pptp{0} = (1,1)$ is the smallest element since $\pptp{1}$ was arbitrary.

    Now we show that every other element of $\pints$ has an immediate predecessor in this order.
    So consider any $\pptp{1} \in \pintss$ where $\pptp{1} \neq (1,1)$.
    Hence either $x_1 \neq 1$ or $y_1 \neq 1$.

    Case: $y_1 = 1$.
    Then it has to be that $x_1 \neq 1$ so that $x_1 > 1$.
    We claim that $\pptp{0} = (1, x_1-1)$ is the immediate predecessor of $\pptp{1}$.
    First we note that clearly $\pptp{0} \in \pintss$ since $x_1 > 1$.
    We also have that $x_0 + y_0 = 1 + x_1 - 1 = x_1 < x_1 + y_1$ since $0 < 1 = y_1$, and so $\pptp{0} < \pptp{1}$.
    Now suppose that there is a point $\pptp{2} \in \pintss$ where $\pptp{0} < \pptp{2} < \pptp{1}$.
    ...
    So it must be that $x_2+y_2 < x_1+y_1 = x_1+1$ since $\pptp{2} < \pptp{1}$.
    Now, since also $\pptp{0} < \pptp{2}$, it must be that $x_0+y_0 \leq x_2+y_2$, but then we have $x_1 = 1 + (x_1 - 1) = x_0 + y_0 \leq x_2+y_2 < x_1+1$, which is impossible.
    So it has to be that no such $\pptp{2}$ exists so that $\pptp{0}$ is the immediate predecessor of $\pptp{1}$.

    Case: $y_1 \neq 1$.
    Then it has to be that $y_1 > 1$ so that the point $\pptp{0} = (x_1+1,y_1-1)$ is still an element of $\pintss$.
    We show that $\pptp{0}$ is the immediate predecessor of $\pptp{1}$.
    First we have that $x_0+y_0 = (x_1+1) + (y_1-1) = x_1 + y_1$ and $y_0 = y_1-1 < y_1$ so that $\pptp{0} < \pptp{1}$.
    Now suppose that there a point $\pptp{2} \in \pintss$ where $\pptp{0} < \pptp{2} < \pptp{1}$.
    ...
    So no such $\pptp{2}$ can exists, and hence $\pptp{0}$ is the immediate predecessor of $\pptp{1}$.

    This in all cases $\pptp{1}$ has an immediate predecessor, which shows the desired result since $\pptp{1} \neq (1,1)$ was arbitrary.
  }
  ...
  \qproof{
    ...
    Thus we can choose an element $\pptp{1}$ of $\pintss$ that has no immediate predecessor in $<$ but also such that $f\pptp{1} \neq (1,1)$ so that $f\pptp{1}$ does have an immediate predecessor in $\prec$.
    So let $(u_0, v_0)$ be the immediate predecessor of $f\pptp{1}$ in $\prec$ and set $\pptp{0} = \inv{f}(u_0,v_0)$ so that $(u_0,v_0) = f\pptp{0}$.
    Then of course $\pptp{0} < \pptp{1}$ since $f\pptp{0} = (u_0,v_0) \prec f\pptp{1}$ and $f$ preserves order.
    But since $\pptp{1}$ has no immediate predecessor in $<$, there is a point $\pptp{2}$ such that $\pptp{0} < \pptp{2} < \pptp{1}$.
    We then have that $(u_0,v_0) = f\pptp{0} \prec f\pptp{2} \prec f\pptp{1}$ since $f$ preserves order, which is a contradiction since $(u_0,v_0)$ is the immediate predecessor of $f\pptp{1}$.
...
\sol{
  ...
  \qproof{
    ...
    Suppose that $A$ is a nonempty subset of $X$ that is bounded above, and that $\pptt{1}$ is an upper bound of $A$ in $X$.
    Then of course we have $x_1 \in [0,1]$ (or $x_1 \in \clop{0,1}$).
    Define $A_x = \braces{x \where \pptt{} \in A}$ so that clearly $A_x \ss [0,1]$ (or $A_x \ss \clop{0,1}$).
    It then follows that $x \leq x_1$ for any $x \in A_x$ since $\pptt{1}$ is an upper bound of $A$ in the dictionary order.
    ...
    Consider any $\pptt{} \in A$ so that $x \in A_x$, and hence $x \leq a$ since $a$ is the least upper bound of $A_x$.
    If $x < a$ then of course $\pptt{} \leq a \times b$, so assume that $x = a$.
    Then $a = x \in A_x$ so that $b$ was defined as the least upper bound of $A_y$.
    Then we have that $y \in A_y$ since $\pptt{} = a \times y \in A$, and thus $y \leq b$ since $b$ is the least upper bound of $A_y$.
    This shows that $\pptt{} \leq a \times b$ so that we  have shown that $a \times b$ is an upper bound of $A$ since $\pptt{}$ was arbitrary.

    To show that it is the \emph{least} upper bound consider any $\pptt{0} \in X$ where $\pptt{0} < a \times b$.
    ...
    Then, since $x \in A_x$, there is a $y \in [0,1]$ where $\pptt{} \in A$.
    Hence $\pptt{0} < \pptt{}$ since $x_0 < x$ so that $\pptt{0}$ is not an upper bound of $A$.
    ...
    Hence $a \times y \in A$, $x_0 = a$, and $y_0 < y$ so that $\pptt{0} < a \times y \in A$, which shows that $\pptt{0}$ is not an upper bound of $A$.
    Thus in either case $\pptt{0}$ is not an upper bound of $A$, which shows that $a \times b$ is the least upper bound since $\pptt{0} < a \times b$ was arbitrary.
  ...
  \qproof{
    ...
    So let $\pptt{1}$ be any upper bound of $A$ in $X$ and suppose for the moment that $x_1 = 0$.
    Then $y_1 \in \clop{0,1}$ so that $0 \leq y_1 < 1$, but then there is a $y_0 \in \reals$ such that $0 \leq y_1 < y_0 < 1$.
    Then $y_0 \in \clop{0,1}$ so that $x_1 \times y_0 = 0 \times y_0 \in A$, but also $\pptt{1} < x_1 \times y_0$ so that $\pptt{1}$ cannot be an upper bound of $A$.
    So it must be that in fact $x_1 \neq 0$ and hence $x_1 > 0$.
    Now let $x_0 = x_1/2 > 0$ and $y_0 = 0$ so that clearly $\pptt{} < \pptt{0} < \pptt{1}$ for any $\pptt{} \in A$ since $x = 0 < x_1/2 = x_0 < x_1$.
    This shows that $\pptt{0}$ is still an upper bound of $A$ but that $\pptt{0} < \pptt{1}$.
    Since $\pptt{1}$ was an arbitrary upper bound of $A$, this proves that $A$ can have no \emph{least} upper bound!
  }
}
