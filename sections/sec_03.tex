...
\sol{
  ...
  \qproof{
    ...
    An analagous argument shows that the immediate predecessor, if it exists, is also unique.
    ...
    An analagous argument shows any largest element is also unique.
  }
}
...
\sol{
  First, for the dictionary order, we claim that every point has an immediate predecessor except points in the subset $A = \braces{\ppt{} \where y = 1}$.
  ...
  \qproof{
    ...
    But then we have $y_0 < y_2 < y_1 = y_0+1$, which is not possble since $y_1 = y_0+1$ is the immediate successor of $y_0$ in $\pints$ so that there can be no integers between them.
    ...
    Since $\ppt{1} \in A$ was arbitrary, this completes the proof that every element in $\pintss$ has an immediate predecessor except thos in $A$.
  ...
  For the order in part~(ii), we claim that every element has an immediate predecessor except those in the subset $A = \braces{\ppt{} \where x=1 \text{ or } y=1}$.
