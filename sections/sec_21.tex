...
\sol{
  ...
  \qproof{
    ...
    Since the absolute value function is continuous on $\reals$, it follows from Lemma~\ref{lem:metric:contseq} and Exercise~21.5 that the sequence $\abs{s_k(x) - s_n(x)}$ converges as $k \to \infty$, and moreover converges to $\abs{s(x) - s_n(x)}$ since $s_k(x) \to s(x)$ as $k \to \infty$ as shown above.
