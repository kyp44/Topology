...
\subsection{Supplementary Exercises: Well-Ordering}
...
\sol{
  (a)
  \qproof{
    For any $(A,<) \in \cA$ we have that it is not equal to a section of itself since then it would then clearly have the same order type as its own section, which would violate Exercise~\secl.2 part (b).
...
\sol{
  (a)
  \qproof{
    Since $(T_1,<_1)$ and $(T_2,<_2)$ are both well-ordered sets, it follows from Exercise~\secl.4 part (a) that either they have the same order type, $T_1$ has the same order type as a section of $T_2$, or vice-versa.
    ...
    Given this, it was shown in the proof of Exercise~\secl.2 part (a) that $h(S_x(T_1)) = S_{h(x)}(T_2)$ for all $x \in T_1$.
...
\sol{
  ...
  \qproof{
    ...
    Then clearly it cannot be that $[(A,<)] \ll [(A,<)]$, since this would mean that $A$ has the same order type as a section of itself, which would contradict what was shown in Exercise~\secl.2 part (b).
    ...
    Then, by Exercise~\secl.4 part (a), it must be that either $(A,<)$ has the same order type as a section of $(A',<')$ or vice-versa.
  ...
  \qproof{
    ...
    This was shown not to be possible in Exercise~\secl.2 part (b) so that a contradiction has been reached.
