...
\sol{
  (a)
  \qproof{
    First note that clearly the inverse of the identity function is itself with the domain and image reversed, and that for any subset $A \ss X = X'$ we have $i(A) = \inv{i}(A) = A$.
  ...
  (b)
  \qproof{
    Clearly $i$ is a bijection since its domain and image are the same set, and $\inv{i} = i$.
    We then have that
    \ali{
...
\exercise{7}{
  ...
  \item Suppose that $f: \reals \to \reals$ is ``continuous from the right,'' that is
    ...
    for each $a \in \reals$.
    Show that $f$ is continuous when considered as a function from $\realsl$ to $\reals$.
  \item Can you conjecture what functions $f: \reals \to \reals$ are continuous when considered as maps from $\reals$ to $\realsl$?
    We shall return to this question in Chapter~3.
  }
}
\sol{
  \begin{lem}\label{lem:cont:Rlopcl}
    In the topology $\realsl$, every basis element is both open and closed.
  \end{lem}
  \qproof{
  ...
  \qproof{
    ...
    So let $U = \clop{a, a+\d}$ which is clearly a basis element of $\realsl$ and contains $a$ so that it is a neighborhood of $a$.
  ...
  (b) We claim that only constant functions are continuous from $\reals$ to $\realsl$.
  \qproof{
    First, it was shown in Theorem~18.2 part~(a) that constant functions are always continuous regardless of the topologies.
    Hence we must show that any continuous function from $\reals$ to $\realsl$ is constant.
    ...
    So consider the basis element $B = \clop{f(0), f(x)}$ of $\realsl$, which clearly contains $f(0)$ but not $f(x)$.
  ...
  Lastly, we claim that  the only functions that are continuous from $\realsl$ to $\realsl$ are those that are \emph{continuous and non-decreasing from the right}.
  ...
  \qproof{
    ...
    We then have that $U = \clop{x, x+\d}$ is a basis element and therefore an open set of $\realsl$ that contains $x$.
...
\sol{
  ...
  \qproof{
    ...
    Thus $f(x)$ is in the image of $f \rest A$ so that there is a neighborhood $U_A$ of $x$ in the subspace topology such that $(f \rest A)(U_A) \ss V$ by Theorem~18.1 since $f \rest A$ is continuous.
