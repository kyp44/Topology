...
\sol{
  ...
  \qproof{
    ...
    Thus we have $f(U_0) = \peqa{0} \neq \peqa{1} = f(U_1)$, which shows that $f$ s injective.
...
\exercise{5}{
  ...
  Show that if $A$ is open in $X$, then the map $q: A \to p(A)$ obtained by resrtricting $p$ is an open map.
}
\sol{
...
\sol{
  ...
  \qproof{
    ...
    If $U = K$ then clearly $\ivp(\braces{U}) = K$, which we know is closed in $\reals_K$ as was shown in Exercise~17.16 part~(b).
    If $U \neq K$ then $U = \braces{x}$ for some $x \in \reals - K$ so that of course $\ivp(\braces{U}) = \braces{x}$ is closed in $\reals_K$ since $\reals_K$ was shown to satisfy the $T_1$ axiom, again in Exercise~17.16 part~(b).
    Hence either way $\ivp(\braces{U})$ is closed in $\reals_K$ so that $\braces{U}$ must be closed in $Y$ since $p$ is a quotient map.
    This suffices to show that $Y$ satisfies the $T_1$ axiom since $U$ was arbitrary.

    To show that $Y$ is not Hausdorff
  }
}
