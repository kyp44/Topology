...
\sol{
  ...
  \begin{lem}\label{lem:metric:unihaus}
    For an index set $J$, the uniform topology on $\reals^J$ is a Hausdorff space.
  \end{lem}
  \qproof{
    Suppose that $\vx$ and $\vy$ are distinct points in $\reals^J$.
    Then there must be an $\a \in J$ where $x_\a \neq y_\a$.
    Since $\reals$ is Hausdorff there are open sets $U$ and $V$ of $\real$ that contains $x_\a$ and $y_\a$, respectively, and that are disjoint.
    Lemma~\ref{lem:metric:ball} then implies that there are $\e,\d > 0$ where $B_d(x_\a,\e)$ and $B_d(y_\a,\d)$ (where $d$ is the standard metric on $\reals$) are subsets of $U$ and $V$, respectively, and therefore must be disjoint.
    Let $\g = \min\braces{1, \e, \d}$ so that clearly $B_{\br}(\vx, \g)$ and $B_{\br}(\vy, \g)$ are neighborhoods of $\vx$ and $\vy$, respectively, in the uniform topology.

    Suppose for the moment that there is a $\vz \in B_{\br}(\vx, \g) \cap B_{\br}(\vy, \g)$.
    Therefore
    \gath{
      \br(\vz, \vx) = \sup\braces{\bd(z_\b, z_\b) \where \b \in J} < \g
    }
    since $\vz \in B_{\br}(\vx, \g)$, where of course $\bd$ is the usual bounded metric on $\reals$.
    It of course has to be that in particular $\bd(z_\a, x_\a) < \g \leq 1$ so that it has to be that $\bd(z_\a, x_\a) = d(z_\a, x_\a)$.
    Then we have $d(z_\a, x_\a) = \bd(z_\a, x_\a) < \g \leq \e$ so that $z_\a \in B_d(x_\a, \e)$.
    An analogous argument with $\vy$ in place of $\vx$ and $\d$ in place of $\e$ shows that also $z_\a \in B_d(y_\a, \d)$.
    However, this is impossible as we know that $B_d(x_\a, \e)$ and $B_d(y_\a, \d)$ are disjoint!
    So it has be that the neighborhoods $B_{\br}(\vx, \g)$ and $B_{\br}(\vy, \g)$ are actually disjoint, which suffices to show that the uniform topology is Hausdorff as desired.
  }
