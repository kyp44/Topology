...
\sol{
  ...
  \qproof{
    ...
    as desired.
    Note that we have used the well known property of sequences that, if $\seqinf{a}$ and $\seqinf{b}$ are real sequences that converge to $a$ and $b$, respectively, then $\lim_{n \to \infty} (a_n + b_n) = a + b$.
...
\sol{
  ...
  \qproof{
    ...
    For part (3) define the function $f(x) = x/(1+x)$ over the domain $\braces{x \in \reals \where x \geq 0}$.
