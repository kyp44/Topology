...
\sol{
  ...
  (d)
  \qproof{
    First recall that the euclidean mertric on $\reals^n$ is defined as
    \gath{
      d(\vx, \vy) = \norm{\vx - \vy} \,.
    }
...
\def\vyb{\overline{\vy}}
\exercise{10}{
  Let $X$ denote the subset of $\reals^\w$ consisting of all sequences $(x_1, x_2, \ldots)$ such that $\sum x_i^2$ converges.
...
\sol{
  ...
  \qproof{
    ...
    To this end we show that the sequence $(s_1, s_2, \ldots)$ is a bounded above.
    ...
    Also, for any $n \in \pints$, define the truncated sequences $\vx_n = (x_1, \ldots, x_n)$ and $\vy_n = (y_1, \ldots, y_n)$.
    We know from Lemmm~\ref{lem:metric:pseries} that the partial series $\sum_{i=n}^\infty x_i^2$ and $\sum_{i=n}^\infty y_i^2$ are non-negative for any $n \in \pints$.
  ...
  \qproof{
    ...
    We also have that $c \vx = (cx_1, cx_2, \ldots)$ by definition, and it again follows from Exercise~21.11 part (b) that
    \gath{
  ...
  \qproof{
    ...
    Referencing Defintion~\ref{def:metric:pseries}, we then have
    \ali{
    ...
    Then we have
    \gath{
      \sum_{i=1}^\infty z_i^2 = \lim_{n \to \infty} \sum_{i=1}^n z_i^2 = \lim_{n \to \infty} \norm{\vz_n}^2 \,.
    }
    ...
    \gath{
      \squares{\sum_{i=1}^\infty z_i^2}^{1/2} = \squares{\lim_{n \to \infty} \norm{\vz_n}^2}^{1/2} = \lim_{n \to \infty} \squares{\norm{\vz_n}^2}^{1/2} = \lim_{n \to \infty} \norm{\vz_n}
    }
  }
}
